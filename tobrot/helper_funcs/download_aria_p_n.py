...
def add_magnet(aria_instance, magnetic_link, c_file_name):
    ...
    try:
        ...
        return False, "**XATO** \n" + str(e) + " \nIltimos menga Tezrok ishlovchi havola manzilini yuboring "
    else:
...
async def call_apropriate_function(
    ...
    for key_f_res_se in final_response:
        ...
        message_to_send += "\n"
    if message_to_send != "":
        mention_req_user = f"<a href='tg://user?id={user_id}'>Siz soragan fayllar</a>\n\n"
        message_to_send = mention_req_user + message_to_send
        message_to_send = message_to_send + "\n\n" + "#uploads"
    else:
        message_to_send = "<i>XATO</i> Faylni yuklab olib bolmadi. 😞😞"
        ...
        if not complete:
            if not file.error_message:
                ...
                try:
                    ...
                    pass
                #
                msg = f"\nFayl Yuklanmoqda: `{downloading_dir_name}`"
                msg += f"\nTezlik: {file.download_speed_string()} 🔽 / {file.upload_speed_string()} 🔼"
                msg += f"\nJarayon: {file.progress_string()}"
                msg += f"\nFayl Hjmi: {file.total_length_string()}"
                # msg += f"\nStatus: {file.status}"
                msg += f"\nQolgan vaqt: {file.eta_string()}"
            ...
            await event.edit(f"Fayl muvofaqqiyatli yuklab olindi: `{file.name}`")
            return True
    except Exception as e:
        LOGGER.info(str(e))
        if " not found" in str(e) or "'file'" in str(e):
            await event.edit("Yuklab olish bekor qilindi :\n`{}`".format(file.name))
            return False
        elif " depth exceeded" in str(e):
            file.remove(force=True)
            await event.edit("Yuklab olish bekor qilindi :\n`{}`\nSizning Torrent Yoki Havolangiz Yaroqsiz.".format(file.name))
            return False
        else:
            LOGGER.info(str(e))
